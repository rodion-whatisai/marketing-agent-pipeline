# Planning — линейка принятия решений

Вход — четыре инстанции (человек-клиент, агент-клиент, человек-стратег, агент-стратег)
сводят один бриф; клиент общается с этим как с единым сервисом. Дальше: микс каналов →
человеческий гейт (эксперт сверяет бриф с бизнес-целью) → построение плана (площадка =
агент) → forecast по позиции → Python-гард по бюджету → медиаплан-простыня. Параллельно
идут треки BI (аудитория) и Creative. Схлопывание простыни, опции бюджета и презентация
клиенту — уже media buying, за пределами этого блока.

```mermaid
flowchart LR
  subgraph IN["Вход · четыре инстанции рождают один бриф"]
    direction TB
    HC["человек-клиент"] --> PR["процессинг"]
    AC["агент-клиент"] --> PR
    HS["человек-стратег"] --> PR
    AS["агент-стратег · дополняет клиентские листы"] --> PR
    PR --> BR["бриф · медиа-KPI + контекст клиента"]
  end
  BR -. ping-pong .-> CS["единый клиентский сервис<br/>одна «сущность» для клиента"]
  BR --> MIX["микс каналов<br/>от присутствия ЦА · история / экспертиза"]
  BI["трек BI · стратегия аудитории<br/>кластеры · темы · подача · позиционирование"] -. параллельно .-> MIX
  MIX --> GATE{"человеческий гейт<br/>эксперт взвешивает бриф<br/>«10k кликов? → 100 продаж»"}
  GATE -->|ок| BUILD["медиаплан: нарезка на сеты<br/>таргет · тип закупки · оптимизация · тема креатива<br/>площадка = агент"]
  CR["трек Creative · запрос форматов и содержания"] -. параллельно .-> BUILD
  BUILD --> FC["forecast по позиции<br/>ёмкость → CPM → impr → CTR → клики → конверсии → стоимость"]
  FC --> PG{"Python-гард<br/>насчитано vs выдано"}
  PG -->|умеренное превышение| ARG["агент: аргументация «почему не $500»"]
  PG -->|грубое ~×50| BLK["блок"]
  PG -->|сходится| OUT["медиаплан-простыня<br/>строчка на позицию"]
  ARG --> OUT
  OUT -. за пределами планинга .-> MB["media buying<br/>схлопывание · min / opt / max · презентация клиенту"]
```

> **Весь блок — спроектирован, в коде ещё нет.** Диаграмма показывает задуманную линейку
> подчинения, не работающий пайплайн.
>
> **Python-гард (бюджет)** — единственный названный детерминированный guardrail: сверяет
> насчитанный бюджет с выданным; умеренное превышение → требует аргументацию, грубое
> (~×50) → блок. Это kill-switch стадии в коде.
>
> **Человеческий гейт** — эксперт ловит подмену прокси-метрики бизнес-целью («10k кликов»
> → «100 продаж») до того, как строится план.
>
> **Параллельные треки** (пунктир) — BI (стратегия аудитории) и Creative (форматы и
> содержание) питают планинг сбоку, не задерживая основную линейку.
>
> **За пределами блока** (пунктир направо): схлопывание каналов в общую простыню, опции
> бюджета min / opt / max и презентация клиенту отнесены к media buying — отдельному шагу
> после планинга.
