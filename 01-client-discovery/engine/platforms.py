...
ШАГ A (2026-07-13): чистая эквивалентность — view-функции обязаны выдавать
байт-в-байт те же таблицы, что были (реперы: test_platforms_frozen.py).
Известные дыры содержимого (Pinterest без PIXEL_RULES, Snapchat без GTM-сигнатур,
мёртвая первая строка Hotjar) НЕ чинятся здесь — это шаг B, отдельным коммитом,
под стендом. Порядок ключей каждого view — часть текущего поведения
(make_listeners матчит первым совпавшим), поэтому порядки заданы явно.
"""
...
PLATFORMS = _build([
    ...
    ("Snapchat", {
        ...
        "noise": ["fired"],
        # gtm_signatures нет — баг B6 (живые snaptr-теги gymshark не репортились);
        # добавляется шагом B
        "shopify_marker": ("snaptr(", "tr.snapchat.com", "sc-static.net"),
    }),
    ("Pinterest", {
        # В PIXEL_RULES Pinterest исторически НЕТ (баг B5: живые ct.pinterest.com
        # хиты gymshark/pipsnacks не регистрировались) — добавляется шагом B.
        "gtm_name": "Pinterest Tag",     # пока не используется (нет сигнатур)
...
_TIER1_ORDER = ["Meta", "Google Analytics", "Google Ads", "Bing/Microsoft",
                "TikTok", "Snapchat"]
_TIER2_ORDER = ["Meta", "Google Analytics", "Google Ads", "Bing/Microsoft", "TikTok"]
