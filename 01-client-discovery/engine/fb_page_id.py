    ...
    if is_handle:
        ...
        if not handles:
            ...
            if fb_total > 0:
                ...
                # Доменный якорь: страница, чьи объявления ведут на сайт клиента.
                # page_profile_uri вида facebook.com/61563178984628/ → page_id из URL.
                # Tested: 2026-07-22 on plurio.ai → 61563178984628 (сошлось с ручной
                # находкой Rodion'а: facebook.com/profile.php?id=61563178984628)
                brand_uris = ads_data.get("brand_page_uris") or []
                brand_page_url = brand_uris[0] if brand_uris else None
                brand_page_id = None
                if brand_page_url:
                    m = re.search(r'facebook\.com/(\d+)', brand_page_url)
                    if m:
                        brand_page_id = m.group(1)
                    if len(brand_uris) > 1:
                        log_warn(f"Доменный якорь дал {len(brand_uris)} страниц: {brand_uris} — беру первую")
                    log_success(f"FB-страница бренда по доменному якорю: {brand_page_url}"
                                + (f" (page_id={brand_page_id})" if brand_page_id else ""), emoji="📎")
