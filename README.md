...
> **Статус по стадиям:** 01 — наполнена (реальный рабочий код + прогоны). 02 и 03 —
> наполнены как дизайн (спроектированы, в коде ещё нет). 04–05 — конспект.
...
Пять стадий. Глубоко проработана одна — 01, на реальном рабочем коде; 02 и 03 показывают,
как выглядит **стадия-дизайн до кода**; 04–05 даны конспективно. На каждой стадии повторяется
...
| **03 — Execution** | двигать бюджет (scale / kill / hold) — тут живут деньги; «проверка невиновности» перед kill | **наполнено как дизайн (не построено)** |
...
| как стадия проектируется **до** кода | [02 — Planning](02-planning/README.md) (+ [формирование брифа](02-planning/brief-formation.md)) · [03 — Execution](03-execution/README.md) — UA-агент, двигающий деньги |
...
- **02** — дизайн, не код: та же граница закладывается *до* реализации; гейт «бриф готов» =
  Python-полнота **и** валидация человеком; всё честно помечено «спроектировано, не построено».
- **03** — агент впервые двигает деньги; почти вся UA-логика детерминирована (агент тонкий,
  человек на необратимом); перед kill — «проверка невиновности» (атрибуция / сайт / сток).
...
задокументирован по реальному рабочему коду (в `01-client-discovery/engine/`); 02 честно
помечен как спроектированный, но не построенный. Где сказано «планируется / в коде ещё
нет» — так и передавай.
